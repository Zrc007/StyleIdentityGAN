...
def load_style_images(style_images: typing.List[str],
        style_images_aligned_dir: str, inversion_codes_dir: str, test_input: StyleImage) \
    ...
    inputs.append(StyleImage.aligned2target(test_input.aligned).to(device))
    input_latents.append(test_input.latent.to(device))

    targets = torch.stack(targets, 0)
    latents = torch.stack(latents, 0)
    inputs = torch.stack(inputs, 0)
    input_latents = torch.stack(input_latents, 0)

    return targets, latents, inputs, input_latents
...
def train(targets: torch.Tensor, latents: torch.Tensor, inputs: torch.Tensor, input_latents: torch.Tensor, options: TrainOptions, args) -> None:
    ...
    for idx in tqdm(range(num_iter)):
        ...
        ##img_showed = transforms.ToPILImage()(utils.make_grid(img, normalize=True, range=(-1, 1)))
        ##wandb.log(
        ##    {"original generator": [wandb.Image(img_showed)]},
        ##    step=idx)
        loss = lpips_fn(F.interpolate(img, size=(256,256), mode='area'),
            F.interpolate(targets, size=(256,256), mode='area')).mean()
        
        input_img = generator(input_latents, input_is_latent=True)
        my_input = transforms.ToPILImage()(utils.make_grid(input_img, normalize=True, range=(-1, 1)))
        #wandb.log(
        #    {"input generate": [wandb.Image(my_input)]},
        #    step=idx)
        loss_feature = lpips_fn(F.interpolate(input_img, size=(256,256), mode='area'),
            F.interpolate(inputs, size=(256,256), mode='area')).mean()
        w_ft = 0.002
        loss = loss + loss_feature * w_ft

        if use_wandb:
            wandb.log({"loss": loss}, step=idx)
            if idx % log_interval == 0 and test_input:
                #print("id:" + str(idx) + " loss:" + str(loss) + " loss_f:" + str(loss_feature))
                generator.eval()
        ...
        loss.backward()
        g_optim.step()
    name_1, _ = os.path.splitext(os.path.basename(args.test))
    name_2, _ = os.path.splitext(os.path.basename(args.images[0]))
    result_path = os.path.join(args.outdir, name_2, f'{name_1}.jpeg')
    print(result_path)
    my_sample.save(result_path)
...
def _main():
    ...
    targets, latents, inputs, input_latents = load_style_images(args.images, style_images_aligned_dir, inversion_codes_dir, test_input)
    ...
    print("train ...")
    train(targets, latents, inputs, input_latents, TrainOptions(
